...
export class SongInfoComponent implements OnInit, AfterViewInit {
  public cover = 'assets/images/player_cover.png';
  ...
  showSongLyric() {
    ...
    // 获取歌词
    if (this.currentSong && this.currentSong.lyric.id && this.currentSong.source) {
      this.ajaxService.getSongLyric(this.currentSong.lyric.id, this.currentSong.source).then((lyric: Lyric) => {
   ...
   * 歌曲加载完后的回调函数
   */
  lyricCallback(lyric: Lyric) {
    let _lyric = this.parseLyric(lyric.sourceLyric);
    if (!_lyric) {
      this.lyric = null;
